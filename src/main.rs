...
fn main() -> Result<(), DocumentError>{
    ...
    let args = App::new("rfz").version("0.1.0")
                              .author("Ben Maddison")
...
fn index(args: &ArgMatches) -> Result<(), DocumentError> {
    ...
    for result in collection.to_map()?
                            .newest(1)
                            .with_threads(jobs)
...
fn sync(args: &ArgMatches) -> Result<(), DocumentError> {
    ...
    let status = Command::new(command).arg("--archive")
                                      .arg("--compress")
                                      .arg("--progress")
                                      .arg("--include=*.html")
                                      .arg("--exclude=**")
